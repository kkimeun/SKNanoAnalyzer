...
void Tutorial_reco_tt::initializeAnalyzer() {
  std::cerr
      << "[DEBUG INIT 0] enter initializeAnalyzer"
      << " | DataEra=" << DataEra
  ...
  std::cerr
      << "[DEBUG INIT 2] trigger configuration complete"
      << " | muTrig=" << IsoMuTriggerName
      << " | muSafePt=" << TriggerSafePtCut
      << " | eleTrig=" << EleTriggerName
      << " | eleSafePt=" << EleTriggerSafePtCut
      << std::endl;

  std::cerr << "[DEBUG INIT 3] before MyCorrection constructor" << std::endl;
  ...
  std::cerr << "[DEBUG INIT 6] after SetTaggingParam" << std::endl;
  ...
  std::cerr << "[DEBUG INIT 8] after SystematicHelper" << std::endl;
  std::cerr << "[DEBUG INIT 9] leave initializeAnalyzer" << std::endl;

}

void Tutorial_reco_tt::executeEvent() {

  static long long debug_event_counter = 0;
  const long long debug_event = debug_event_counter++;

  if (debug_event < 5) {
    std::cerr
        << "[DEBUG EVT 0] enter executeEvent"
        << " | event index=" << debug_event
        << std::endl;
  }

  AllMuonViews = GetAllMuonViews();
  if (debug_event < 5) {
    std::cerr << "[DEBUG EVT 1] got muon views" << std::endl;
  }

  AllElectronViews = GetAllElectronViews();
  if (debug_event < 5) {
    std::cerr << "[DEBUG EVT 2] got electron views" << std::endl;
  }

  AllJetViews = GetAllJetViews();
  if (debug_event < 5) {
    std::cerr << "[DEBUG EVT 3] got jet views" << std::endl;
  }

  AllGenViews = GetAllGenViews();
  AllGenJetViews = GetAllGenJetViews();
  if (debug_event < 5) {
    std::cerr << "[DEBUG EVT 4] got gen views" << std::endl;
  } 

  ev = GetEvent();
  if (debug_event < 5) {
    std::cerr << "[DEBUG EVT 5] got event" << std::endl;
  } 
  ...
  weight_Prefire = 1.; 
  if (debug_event < 5) {
    std::cerr << "[DEBUG EVT 6] before systematic loop" << std::endl;
  }
  for (const auto& syst_dummy : *systHelper) {
    ...
    if (debug_event < 5) {
      std::cerr
          << "[DEBUG EVT 7] before executeEventFromParameter"
          << std::endl;
    }
    ...
    if (debug_event < 5) {
      std::cerr
          << "[DEBUG EVT 8] after executeEventFromParameter"
          << std::endl;
    }
  }

  if (debug_event < 5) {
    std::cerr << "[DEBUG EVT 9] leave executeEvent" << std::endl;
  }
}
...
void Tutorial_reco_tt::executeEventFromParameter() {
  ...
  std::cerr
      << "[DEBUG PARAM 0] enter executeEventFromParameter"
      << std::endl;
  ...
  std::cerr
    << "[DEBUG PARAM 1] current systematic = "
    << this_syst
    << std::endl;
  ...
  if (IsDATA) {
    ...
    std::function<float(MyCorrection::variation, TString)> muon_id_lambda =
        [&](MyCorrection::variation syst, TString source) {
          ...
          std::cerr
            << "[DEBUG WEIGHT] before Muon_ID"
            << " | key=" << this_muon_id_sf_key
            << std::endl;

          const float result =
            myCorr->GetMuonIDSF(this_muon_id_sf_key, muons, syst);

          std::cerr
            << "[DEBUG WEIGHT] after Muon_ID"
            << " | result=" << result
            << std::endl;
    ...
    std::function<float(MyCorrection::variation, TString)> electron_id_lambda =
        [&](MyCorrection::variation syst, TString source) {
          ...
          if (!is_electron_channel) return 1.f;
            std::cerr
            << "[DEBUG WEIGHT] before Electron_ID"
            << " | key=" << this_electron_id_sf_key
            << std::endl;

          const float result =
            myCorr->GetElectronIDSF(
              this_electron_id_sf_key,
              electrons,
              syst
            );

          std::cerr
            << "[DEBUG WEIGHT] after Electron_ID"
            << " | result=" << result
            << std::endl;
    ...
    std::function<float(MyCorrection::variation, TString)> electron_reco_lambda =
        [&](MyCorrection::variation syst, TString source) {
          ...
          if (!is_electron_channel) return 1.f;
          std::cerr
            << "[DEBUG WEIGHT] before Electron_Reco"
            << std::endl;

          const float result =
            myCorr->GetElectronRECOSF(electrons, syst);

          std::cerr
            << "[DEBUG WEIGHT] after Electron_Reco"
            << " | result=" << result
            << std::endl;
    ...
    std::function<float(MyCorrection::variation, TString)> electron_trig_lambda =
        [&](MyCorrection::variation syst, TString source) {
          ...
          if (!is_electron_channel) return 1.f;
          std::cerr
            << "[DEBUG WEIGHT] enter Electron_Trig"
            << " | DataEra=" << DataEra
            << " | key=" << this_electron_trig_sf_key
            << std::endl;

          if (DataEra == "2024") {
            std::cerr
              << "[DEBUG WEIGHT] skip Electron_Trig for 2024"
              << std::endl;
            return 1.f;
          }
          ...
          std::cerr
            << "[DEBUG WEIGHT] before GetElectronTriggerSF"
            << std::endl;

          const float result =
            myCorr->GetElectronTriggerSF(
              this_electron_trig_sf_key,
              electron.Eta(),
              electron.Pt(),
              electron.Phi(),
              syst
          ...
          std::cerr
            << "[DEBUG WEIGHT] after GetElectronTriggerSF"
            << " | result=" << result
            << std::endl;
    ...
    std::cerr
      << "[DEBUG CALC 0] before assignWeightFunctionMap"
      << std::endl;

    systHelper->assignWeightFunctionMap(weight_function_map);
    std::cerr
      << "[DEBUG CALC 1] after assignWeightFunctionMap"
      << std::endl;

    std::cerr
      << "[DEBUG CALC 2] before calculateWeight"
      << std::endl;
    
    weight_map = systHelper->calculateWeight();
    std::cerr
      << "[DEBUG CALC 3] after calculateWeight"
      << " | weight_map size=" << weight_map.size()
      << std::endl;
