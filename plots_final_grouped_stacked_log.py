...
output_dir = os.path.join(base_dir, "compare_data_grouped_mc_stack_withW_log")
...
# colors
# =========================================================
TT_COLOR = ROOT.TColor.GetColor("#5790fc")         # blue
SINGLETOP_COLOR = ROOT.TColor.GetColor("#f89c20")  # orange
WJETS_COLOR = ROOT.TColor.GetColor("#964a8b")      # purple-ish
...
def get_positive_minimum_from_stack_inputs(group_hists, h_data):
    vals = []

    for h, _ in group_hists:
        m = get_positive_minimum(h)
        if m is not None:
            vals.append(m)

    m_data = get_positive_minimum(h_data)
...
def draw_stack_with_ratio_logy(h_data, group_hists, h_mc_total, out_path, label=""):
    ...
    y_max = max(h_data.GetMaximum(), h_mc_total.GetMaximum()) * 20.0
    y_min = get_positive_minimum_from_stack_inputs(group_hists, h_data) * 0.2
    if y_min <= 0:
        y_min = 1e-3
